...
class ClusteringLayer(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        """
        # compute soft assignments via student’s t-distribution
...
class DEC(object):
    ...
    def pretrain(self, x, y=None, optimizer='adam', epochs=200, batch_size=256, save_dir='results/temp'):
        ...
        weights_path = os.path.join(save_dir, 'ae_weights.weights.h5')
        self.autoencoder.save_weights(weights_path)
        print(f'Pretrained weights are saved to {weights_path}')
    ...
    def fit(self, x, y=None, maxiter=2e4, batch_size=256, tol=1e-3,
        ...
        for ite in range(int(maxiter)):
            ...
            if ite % save_interval == 0:
                weights_file = save_dir + '/DEC_model_' + str(ite) + '.weights.h5'
                print('saving model to:', weights_file)
        ...
        logfile.close()
        final_weights = save_dir + '/DEC_model_final.weights.h5'
        print('saving model to:', final_weights)
        self.model.save_weights(final_weights)

        return y_pred

    def fit_predict(self, x, y=None, maxiter=2e4, batch_size=256, tol=1e-3,
                    update_interval=140, save_dir='./results/temp'):
        """
        Fit DEC model and return predicted labels (alias for `fit`).
        """
        return self.fit(x, y=y, maxiter=maxiter, batch_size=batch_size,
